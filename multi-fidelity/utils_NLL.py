...
def negative_loglikelihood_jnp(hyper, X, Y, kernel_f=True):
...
def negative_loglikelihood(hyper, X, Y, kernel_f=True):
        #pre-data
        n_point, nx_dim   = X.shape[0], X.shape[1]
...
def determine_hyperparameters(X, Y, kernel_f=True, noise = None):
        '''
